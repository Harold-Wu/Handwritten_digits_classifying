...
view_digit <- function(file_type = NULL, observation_number = NULL) {
    ...
    digit_matrix <- digit_matrix[,nrow(digit_matrix):1]    #turn the matrix upside down.
...
for (i in 0:9) {
    ...
    digit_mean <- digit_mean[,nrow(digit_mean):1]    #turn the matrix upside down.
...
predict_knn <- function (test_points = NULL, train_points = NULL, Minkowski_p = 2, k = 3) {
    ...
    predict_result <- matrix(nrow = test_nrow, ncol = k+1)    #build empty matrix to record the k nearest distance for each test obs.
    ...
    for (i in 1:test_nrow) {
        ...
        predict_result[i,2:(k+1)] <- test_distance[order(test_distance[,2])[1:k],1]    #sort and fill in the k labels with k nearest distance.
        colnames(predict_result) <- c("real digit", paste("d", 1:k, sep = ""))    #change colname: the smallest number after "d" indicate the shortest distance.
    }
